...
- registered ideas: `ideas/i###_*`
- raw research packets: `ideas/research_packets/*.md`
...
ideas/INDEX.md
ideas/TODO.md
ideas/research_packets/CATALOG.md
```
...
| Need | Source |
|---|---|
| What is implemented or ready to implement | `ideas/registry.jsonl` and `ideas/i###_*/idea.yaml` |
| Raw imported ChatGPT/Deep Research output | `ideas/research_packets/` |
| Implementation/performance TODO | `ideas/TODO.md` |
| Full packet catalog | `ideas/research_packets/CATALOG.md` |
| Machine-readable packet catalog | `ideas/research_packets/CATALOG.jsonl` |
| Deep Research duplicate-memory prompt | `ideas/chatgpt_pro_deep_math_research_prompt.md` |
| Generated compact idea prompt | `ideas/idea_generation_prompt.md` |
| Benchmark reporting standard | `ideas/BENCHMARK_REPORTING.md` |

Do not treat a raw research packet as implemented. Do not add a packet to `registry.jsonl` until it has a complete `ideas/i###_*` folder.

## Lifecycle

| Stage | Meaning | Required action |
|---|---|---|
| `packet` | Raw research note in `research_packets/`. | Catalog it and update prompt duplicate memory if meaningful. |
...
1. Pick one packet from `ideas/research_packets/CATALOG.md`.
2. Check it against `ideas/chatgpt_pro_deep_math_research_prompt.md` duplicate-memory sections.
3. Choose the next available ID:

```bash
find ideas -maxdepth 1 -type d -name 'i[0-9][0-9][0-9]_*' | sort
```

4. Copy `ideas/idea_template/` to `ideas/i###_<slug>/`.
...
6. Append one JSON line to `ideas/registry.jsonl`.
...
3. Add a short run note under `ideas/i###_*/runs/`.
...
1. Copy only relevant Markdown research packets into `ideas/research_packets/`.
...
4. Update `ideas/research_packets/README.md` with a compact import summary.
5. Update `ideas/chatgpt_pro_deep_math_research_prompt.md` so future research avoids near-duplicates.
6. Regenerate catalogs and prompts:

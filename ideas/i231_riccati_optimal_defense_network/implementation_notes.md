...
- Registry key: `riccati_optimal_defense_network`.
...
- The custom linear-algebra operator described in the source packet is NOT yet
  ...
  `packet_profile=riccati_optimal_defense_network` so that the idea passes the registry contract
  and runs on the standard puzzle_binary benchmark. To upgrade to a bespoke module,
  add `src/chess_nn_playground/models/riccati_optimal_defense_network.py`, register a builder in

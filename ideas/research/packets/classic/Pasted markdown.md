...
Include `ideas/<idea_id>_<slug>/...`, `src/chess_nn_playground/models/<model_name>.py`, `src/chess_nn_playground/models/registry.py`, `configs/<config_name>.yaml`, focused tests if needed, and `ideas/research/prompts/chatgpt_pro_deep_math_research_prompt.md`.

Codex must update `ideas/research/prompts/chatgpt_pro_deep_math_research_prompt.md` after consuming your output. It should preserve hard constraints while adding reusable lessons, anti-duplicate rules, clearer output requirements, or failure-mode guidance discovered from this research pass.

...
The `event_delta_bilinear_accumulator` mixer computes the FM-identity
triple `[A; B; Q]` via three `O(64 d)` sparse-set sums (no Python
loop), then a per-square fusion MLP `Linear(C + 3d -> C) -> GELU ->
Linear(C -> C)`. The dominant per-block cost is the U/V projections
and the fusion MLP, both `O(64 C d)`; the pair-term FM identity
itself is only `O(C d)` after the sums. Expect the per-block FLOP
count to sit close to a 3x3 conv and the wall-clock cost to sit
between the `conv` and `attention` baselines. The shared trainer

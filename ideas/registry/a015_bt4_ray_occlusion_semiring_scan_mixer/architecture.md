...
`BT4 Primitive Mixer (ray_occlusion_semiring_scan)` is a controlled architecture study, not a new
primitive. It holds the BT4-style residual tower fixed and swaps only the
per-block spatial-mixing operator.

## Implementation Binding

- Registered model name: `bt4_ray_occlusion_semiring_scan_mixer` (alias of `bt4_primitive_mixer`
...
convs. Here that mixer is replaced by the `ray_occlusion_semiring_scan` primitive, adapted to the
...
this the cleanest available test of "is the `ray_occlusion_semiring_scan` primitive a better

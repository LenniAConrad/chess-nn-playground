...
The `incremental_delta_linear_head` mixer computes a per-square
linear projection `W_s x_s` (an `einsum('bsc,sdc->bsd', ...)` plus
bias), a global sum over the 64 squares to form `S in R^d`, a
`LayerNorm(S)`, and a per-square fusion MLP `Linear(C + d -> C) ->
GELU -> Linear(C -> C)`. The dominant per-block cost is the per-
square projection and the fusion MLP, both `O(64 C d)`; the global
sum and `LayerNorm` are cheap. Expect the per-block FLOP count to
sit close to a 3x3 conv at matched widths and the wall-clock cost
to sit between the `conv` and `attention` baselines. The shared
trainer logs `train_samples_per_second` to `speed_summary.json`;
if throughput on matched hardware falls below ~40% of the `conv`

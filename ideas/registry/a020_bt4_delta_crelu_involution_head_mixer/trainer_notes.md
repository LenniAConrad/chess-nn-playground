...
The DCIH mixer adds a per-square embedding `Linear(C, d)`, two
involutions (channel + rank flip), and a fuse `Linear(4d, C)` per
block. The static cost is dominated by the embedding and fuse; with
`accumulator_dim=64` and `channels=64` the per-block parameter cost is
broadly comparable to a 3x3 conv pair, but FLOPs may differ because the
involution and saturation summary touch the full `(B, 64, C)` token
tensor twice. The shared trainer logs `train_samples_per_second` to

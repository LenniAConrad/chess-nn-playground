...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/delta_crelu_involution_head.py`.
- Idea-local wrapper: `ideas/registry/a020_bt4_delta_crelu_involution_head_mixer/model.py`.
- Registered model alias: `bt4_delta_crelu_involution_head_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=delta_crelu_involution_head`).
- Source primitive idea: `ideas/registry/p015_delta_crelu_involution_head`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `delta_crelu_involution_head`. The tower then
...
## DCIH-specific adaptation

The source primitive (`p015_delta_crelu_involution_head`) was designed
as an additive head over the i193 trunk. Two adaptations were required
to host it as a per-block spatial mixer:

1. The involution `iota` has a clean square half (rank flip `r -> 7 - r`)
   that is reproduced verbatim. The colour-swap half acts on
   `(piece_type, square)`; with arbitrary `C` channels carrying no
   piece-plane semantics, this is implemented as a channel-reversal
   involution `c -> C - 1 - c`. The composition is a true involution
   (`iota^2 = id`) and the channel-agnostic stand-in for plane swap.
2. The static-position trainer evaluates the analytical fixed point of
   the DeltaCReLU recurrence (full forward over the active piece-square
   set). The mixer reproduces the saturation tape with a `ClippedReLU`
   over the per-square embedding sum; the joint-gradient-over-the-tape
   contract collapses to this fixed point.

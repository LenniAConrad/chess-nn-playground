# Architecture — Ray-Semiring χ-Head (p016)
...
3. **Primitive-specific accumulator state**. Implemented in
   ``compute_state`` of ``RaySemiringChiHead``. See
...
- Registered model name: ``ray_semiring_chi_head``.
- Source implementation: ``src/chess_nn_playground/models/primitives/ray_semiring_chi_head.py``.
- Shared helper: ``src/chess_nn_playground/models/primitives/delta_accumulator.py``.
- Idea-local wrapper: ``ideas/registry/p016_ray_semiring_chi_head/model.py``.
- Training config: ``ideas/registry/p016_ray_semiring_chi_head/config.yaml``.
- Builder entry in ``src/chess_nn_playground/models/registry.py``:
  ``MODEL_BUILDERS['ray_semiring_chi_head'] = build_ray_semiring_chi_head_from_config``.

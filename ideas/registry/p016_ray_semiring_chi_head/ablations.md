# Ablations — Ray-Semiring χ-Head (p016)
...
``ray_semiring_chi_head.RaySemiringChiHead.DEFAULT_ABLATIONS``).
...
See ``ray_semiring_chi_head.RaySemiringChiHead.DEFAULT_ABLATIONS`` for the
complete list; the primitive-specific falsifiers documented in
``ideas/research/primitives/external_10_ray_semiring_exchange_and_chi_head.md`` are exposed verbatim.

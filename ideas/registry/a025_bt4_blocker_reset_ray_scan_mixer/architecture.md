...
`BT4 Primitive Mixer (blocker_reset_ray_scan)` is a controlled architecture study, not a new
primitive. It holds the BT4-style residual tower fixed and swaps only the
per-block spatial-mixing operator.

## Implementation Binding

- Registered model name: `bt4_blocker_reset_ray_scan_mixer` (alias of `bt4_primitive_mixer`
  with `mixer=blocker_reset_ray_scan`)
- Tower / source implementation file: `src/chess_nn_playground/models/architecture/bt4_primitive_mixer.py`
- Mixer source: `src/chess_nn_playground/models/architecture/bt4_mixers/blocker_reset_ray_scan.py`
- Idea-local wrapper: `ideas/registry/a025_bt4_blocker_reset_ray_scan_mixer/model.py`
- Source primitive idea: `p020_blocker_reset_ray_scan`
...
convs. Here that mixer is replaced by the `blocker_reset_ray_scan` primitive, adapted to the
...
this the cleanest available test of "is the `blocker_reset_ray_scan` primitive a better

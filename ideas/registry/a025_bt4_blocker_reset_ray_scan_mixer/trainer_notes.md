...
The `blocker_reset_ray_scan` mixer runs a depth-`L` (`L <= 8` after
prepending the source step) Python-loop scan across 8 directions and 64
source squares per block, so the per-call cost is
`O(64 * 8 * L * hidden_dim)` for the recurrence plus a
`Linear(8 * C -> C)` directional fuse. This is comparable to a 3x3 conv
in FLOPs but the sequential ray loop limits CUDA parallelism; expect
the wall-clock cost to sit between the `conv` and `attention` baselines.

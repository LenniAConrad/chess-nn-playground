# Trainer Notes — p010 Ray-Occlusion Semiring Scan
...
- Smoke command:
  ...
  PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python -m pytest \
    tests/test_ray_occlusion_semiring_scan.py
  ```
...
- Static config validation:
  ...
  PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python \
    ...
    ideas/registry/p010_ray_occlusion_semiring_scan/config.yaml
  ```
...
- Falsifier sweep:
  ...
  RUN_PRIMITIVE_TRAIN=1 RUN_PRIMITIVE_DRY_RUN=0 \
    RUN_PRIMITIVE_CONFIGS=ideas/registry/p010_ray_occlusion_semiring_scan/config.yaml \

...
The SDA mixer is cheap per call: one LayerNorm, two Linears (`embed`
and `projection`), a Hardtanh, and a fuse Linear over `2 * d` features.
Wall-clock should be close to the `conv` baseline. The shared trainer
logs `train_samples_per_second` to `speed_summary.json`; if throughput
falls below ~80% of the `conv` baseline on matched hardware,
investigate before relying on matched-budget comparisons.

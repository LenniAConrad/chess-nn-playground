...
The TCC mixer is more expensive per call than a 3x3 conv because it
runs an `O(N^2 * C)` pairwise soft-rank pass per board to construct
the tail concordance matrix. The shared trainer logs

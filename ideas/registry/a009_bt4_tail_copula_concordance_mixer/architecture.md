...
`BT4 Primitive Mixer (tail_copula_concordance)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `tail_copula_concordance` primitive, adapted to the
...
this the cleanest available test of "is the `tail_copula_concordance` primitive a better

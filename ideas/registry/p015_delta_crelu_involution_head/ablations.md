# Ablations — DeltaCReLU + Involution Reynolds Head (p015)
...
``delta_crelu_involution.DeltaCReLUInvolutionHead.DEFAULT_ABLATIONS``).
...
See ``delta_crelu_involution.DeltaCReLUInvolutionHead.DEFAULT_ABLATIONS`` for the
complete list; the primitive-specific falsifiers documented in
``ideas/research/primitives/external_09_delta_crelu_involution_graph_message.md`` are exposed verbatim.

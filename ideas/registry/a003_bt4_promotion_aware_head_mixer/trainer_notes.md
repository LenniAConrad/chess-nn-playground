...
The PAH mixer is moderately more expensive per call than a 3x3 conv:
each block computes a four-branch per-token fanout `(B, 64, 4, C)` and
runs a `4`-way cross-attention softmax per token. The four type
transforms are small Linear+GELU stacks, so the dominant cost grows
linearly in `C` and is independent of board size beyond the fixed 64
tokens. The shared trainer logs `train_samples_per_second` to
...
with FEN; calibration by slice. The promotion / underpromotion entries
under `crtk_tactic_motifs` are the headline slice for this idea and
must be reported with explicit deltas vs `bt4_conv_mixer` and
`bt4_attention_mixer`.

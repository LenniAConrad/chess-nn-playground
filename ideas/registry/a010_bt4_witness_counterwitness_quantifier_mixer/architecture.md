...
`BT4 Primitive Mixer (witness_counterwitness_quantifier)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `witness_counterwitness_quantifier` primitive, adapted to the
...
this the cleanest available test of "is the `witness_counterwitness_quantifier` primitive a better

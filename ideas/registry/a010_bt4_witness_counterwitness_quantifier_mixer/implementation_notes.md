...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/witness_counterwitness_quantifier.py`.
- Idea-local wrapper: `ideas/registry/a010_bt4_witness_counterwitness_quantifier_mixer/model.py`.
- Registered model alias: `bt4_witness_counterwitness_quantifier_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=witness_counterwitness_quantifier`).
- Source primitive idea: `ideas/registry/p005_witness_counterwitness_quantifier`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `witness_counterwitness_quantifier`.
The tower then constructs `N` `BT4MixerBlock`s, each of which builds
the named mixer through the `bt4_mixers.build_mixer` factory. The
mixer is required by the BT4 block to be shape-preserving:
...
The WCQ mixer pools the 64 board squares by `K` learnable candidate
queries and `R` learnable reply queries (set-query attention), scores
each candidate with a `claim_head` MLP, scores each
`(candidate, reply)` pair with a bilinear `counter_form`, then
collapses the pair grid through the nested `logsumexp` quantifier.
Witness weights are scattered back onto the 64 squares via the
candidate-compiling attention so the residual update preserves the
`(B, C, 8, 8)` shape.

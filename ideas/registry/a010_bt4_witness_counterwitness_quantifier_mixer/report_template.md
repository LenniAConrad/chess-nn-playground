...
- Extra report sections:
  ...
  - Per-block mixer output norm and effective rank (probe the
    ...
    0..N).
  - Quantifier diagnostics on a held-out batch: distribution of
    `margin = claim - counter_envelope` across candidates, witness
    entropy, and how often the soft `argmax` over witnesses
    concentrates on a single candidate.
...
- Required comparisons:
  ...
  - `p005_witness_counterwitness_quantifier` (A3 head-form
    control).
  - Capacity-matched `bt4_conv_mixer` (A4 control).
- Known blockers:
  - The WCQ mixer is more expensive per call than a 3x3 conv. If
  ...
  - The simple_18 board lacks halfmove/fullmove counters; this is
    ...
    diagnostics if the witness/counterwitness compilation depends on
    move-count context.
  ...
  - The soft-quantifier temperatures are held constant inside the
    tower; if the witness entropy stays near `log(K)` across blocks
    the quantifier never picks a witness and the mixer reduces to
    averaged attention.
...
- Target slices where this idea should beat the strongest baseline:
  high-`crtk_difficulty` puzzles whose resolution rests on a single
  surviving forcing candidate against a tight defender response, and
  `crtk_tactic_motifs` dominated by quiet quiet-move /
  zugzwang-style witnesses where conv and attention should
  under-localize.
- Slices where this idea is expected to fail or merely match:
  `mate_in_1`, opening-`crtk_phase` slices, and motifs where the
  decisive move is obvious from a single-square local pattern --
  these should be measured for non-regression, not for lift.
- Ablation that should erase the slice-level gain: A1 (replace WCQ

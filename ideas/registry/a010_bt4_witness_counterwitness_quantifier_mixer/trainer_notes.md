...
The WCQ mixer is more expensive per call than a 3x3 conv because it
runs two set-query attention pools (candidates, replies) plus a full
bilinear `(K x R)` counter grid per block. With the default
`K=R=16, token_dim=48` this is dominated by the bilinear cost at the
default `num_blocks=4`. The shared trainer logs

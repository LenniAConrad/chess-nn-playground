# Architecture — Signed-Edit Bilinear Memory (p012)
...
3. **Primitive-specific accumulator state**. Implemented in
   ``compute_state`` of ``SignedEditBilinearMemory``. See
...
- Registered model name: ``signed_edit_bilinear_memory``.
- Source implementation: ``src/chess_nn_playground/models/primitives/signed_edit_bilinear_memory.py``.
- Shared helper: ``src/chess_nn_playground/models/primitives/delta_accumulator.py``.
- Idea-local wrapper: ``ideas/registry/p012_signed_edit_bilinear_memory/model.py``.
- Training config: ``ideas/registry/p012_signed_edit_bilinear_memory/config.yaml``.
- Builder entry in ``src/chess_nn_playground/models/registry.py``:
  ``MODEL_BUILDERS['signed_edit_bilinear_memory'] = build_signed_edit_bilinear_memory_from_config``.

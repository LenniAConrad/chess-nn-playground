...
`BT4 Primitive Mixer (sparse_legal_graph_transition)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `sparse_legal_graph_transition` primitive, adapted to the
...
this the cleanest available test of "is the `sparse_legal_graph_transition` primitive a better

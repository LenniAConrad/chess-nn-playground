...
The `occlusion_semiring_ray_scan` mixer computes its exclusive prefix
product via a vectorised `cumsum` over 8 directions and at most 7 ray
steps per source square, then a `Linear(8 * C -> C)` directional fuse.
There is no Python-loop scan: the prefix product is a single
`log -> cumsum -> shift -> exp` chain over a `(B, 8, 64, 7)` tensor and
the per-direction projection is one batched `einsum` against a
`(8, C, C)` weight. Expect the per-block FLOP count to sit close to a
3x3 conv and the wall-clock cost to sit between the `conv` and

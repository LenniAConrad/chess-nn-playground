...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/occlusion_semiring_ray_scan.py`.
- Idea-local wrapper: `ideas/registry/a026_bt4_occlusion_semiring_ray_scan_mixer/model.py`.
- Registered model alias: `bt4_occlusion_semiring_ray_scan_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=occlusion_semiring_ray_scan`).
- Source primitive idea: `ideas/registry/p021_occlusion_semiring_ray_scan`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `occlusion_semiring_ray_scan`. The tower then
...
The source primitive (p021) reads occupancy directly off the simple_18
...
preserves the defining property of the source thesis -- the
transmittance is generated *inside* the operator from the prefix
product of `(1 - O)` and never supplied externally -- but it is a
learned indicator rather than a binary piece-plane read. The math
thesis flags this as one of the documented failure modes to watch
for. Ray geometry itself (the 8 queen-style directions, the per-
direction step indices, and the off-board step mask) is a rule-derived
constant buffer registered at construction time.

The exclusive prefix product is implemented in log-domain:
`T = exp(shifted_cumsum(log(clamp(1 - O, log_eps, 1))))` with a leading
zero pad so step 1 always has `T = 1`. `log_eps = 1e-4` prevents
`log(0)` when an occupancy saturates to 1; the resulting near-zero `T`
matches the desired "blocker zeroes the rest of the ray" behaviour.
A multiplicative `ray_step_mask` zeroes any off-board step so the
8-direction projection sums only over valid cells.

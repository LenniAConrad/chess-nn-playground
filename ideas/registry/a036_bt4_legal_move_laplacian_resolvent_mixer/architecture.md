...
`BT4 Primitive Mixer (legal_move_laplacian_resolvent)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `legal_move_laplacian_resolvent` primitive, adapted to the
...
this the cleanest available test of "is the `legal_move_laplacian_resolvent` primitive a better

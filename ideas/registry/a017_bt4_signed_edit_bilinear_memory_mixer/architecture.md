...
`BT4 Primitive Mixer (signed_edit_bilinear_memory)` is a controlled architecture study, not a new
primitive. It holds the BT4-style residual tower fixed and swaps only the
per-block spatial-mixing operator.

## Implementation Binding

- Registered model name: `bt4_signed_edit_bilinear_memory_mixer` (alias of `bt4_primitive_mixer`
...
convs. Here that mixer is replaced by the `signed_edit_bilinear_memory` primitive, adapted to the
...
this the cleanest available test of "is the `signed_edit_bilinear_memory` primitive a better

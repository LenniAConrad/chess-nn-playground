# Architecture — DeltaState + SLG Diffusion (p018)
...
3. **Primitive-specific accumulator state**. Implemented in
   ``compute_state`` of ``DeltaStateSLGDiffusionHead``. See
...
- Registered model name: ``delta_state_slg_diffusion``.
- Source implementation: ``src/chess_nn_playground/models/primitives/delta_state_slg_diffusion.py``.
- Shared helper: ``src/chess_nn_playground/models/primitives/delta_accumulator.py``.
- Idea-local wrapper: ``ideas/registry/p018_delta_state_slg_diffusion/model.py``.
- Training config: ``ideas/registry/p018_delta_state_slg_diffusion/config.yaml``.
- Builder entry in ``src/chess_nn_playground/models/registry.py``:
  ``MODEL_BUILDERS['delta_state_slg_diffusion'] = build_delta_state_slg_diffusion_from_config``.

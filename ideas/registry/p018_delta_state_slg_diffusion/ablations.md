# Ablations — DeltaState + SLG Diffusion (p018)
...
``delta_state_slg_diffusion.DeltaStateSLGDiffusionHead.DEFAULT_ABLATIONS``).
...
See ``delta_state_slg_diffusion.DeltaStateSLGDiffusionHead.DEFAULT_ABLATIONS`` for the
complete list; the primitive-specific falsifiers documented in
``ideas/research/primitives/external_17_delta_state_slg_diffusion_fg_tp.md`` are exposed verbatim.

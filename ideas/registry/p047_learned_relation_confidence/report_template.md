# Report Template

## Run

- Result path:
- Config:
- Seeds:
- GPU:
- Training budget:
- Reporting standard: `ideas/docs/BENCHMARK_REPORTING.md`
- Validation slice report: `slice_report_val.md`
- Test slice report: `slice_report_test.md`

## Aggregate Metrics

- Accuracy:
- F1:
- ROC AUC:
- PR AUC:
- Calibration:

## Architecture-Specific Diagnostics

- Mechanism family: `sheaf`
- Primitive: LRC (Learned Relation Confidence)
- `lrc_global_mean_confidence` distribution
- `lrc_mask_density` distribution
- Per-relation `lrc_mean_conf_<rel>` distributions (12 relations)
- Per-relation `lrc_kept_<rel>` distributions (12 relations)
- Per-relation `lrc_entropy_<rel>` distributions (12 relations)
- `primitive_gate` mean / max / fraction > 0.5 on:
  - high-density positions (`lrc_mask_density` > median)
  - sparse-relation / endgame positions (`lrc_mask_density` < median)
- `primitive_delta` distribution on the same two buckets

## Required Benchmark Reporting

Follow `ideas/docs/BENCHMARK_REPORTING.md`. Every promoted idea must
report aggregate metrics plus the fine-label diagnostic matrix,
`slice_report_val.md`, `slice_report_test.md`, and performance broken
down by `crtk_difficulty`, `crtk_phase`, `crtk_eval_bucket`,
`crtk_tactic_motifs`, and `crtk_tag_families`. Include per-slice false
positives for fine label `1`, per-slice false negatives for fine label
`2`, confidence/calibration by slice, and the highest-confidence wrong
examples with FEN, difficulty, phase, and motifs.

## Slice Findings

- Target slice: "strength-of-known-relation" puzzles
  - Required: p047 unablated >= i193 + 0.02 PR AUC on target slice
  - Required: A1 (`binary_only`) loses >= 50% of that lift
  - Required: A4 (`gate_only`) loses >= 30% of that lift
- Watch slice: `crtk_eval_bucket = equal` -- must not regress
- Required `crtk_difficulty` breakdown: lift must concentrate on
  medium/hard buckets without regressing the easy bucket.
- Required `crtk_phase` breakdown: lift must hold on middlegame and
  endgame buckets, with no opening-bucket regression.

## Ablation Comparison Table

| Ablation | target slice PR AUC | aggregate PR AUC | gate mean | global mean conf |
|---|---|---|---|---|
| `none` | | | | |
| `binary_only` | | | | |
| `scrambled_mask` | | | | |
| `shuffle_pieces` | | | | |
| `gate_only` | | | | |
| `no_low_rank` | | | | |
| `no_edge_mlp` | | | | |
| `zero_delta` | | | | |
| `trunk_only` | | | | |
| `disable_gate` | | | | |

## Keep / Drop Decision

- [ ] Aggregate PR AUC delta from i193 >= -0.005
- [ ] target slice lift >= +0.02
- [ ] A1 (`binary_only`) loses >= 50% of the lift
- [ ] A4 (`gate_only`) loses >= 30% of the lift
- [ ] `crtk_eval_bucket = equal` slice did not regress
- [ ] Throughput drop versus i193 < 15%

If any box fails: drop p047.

## Conclusions

- What the model appears able to learn:
- What the model appears unable to learn:
- Highest-confidence wrong examples (FEN, difficulty, phase, motifs):
- Recommended next step (promote / drop):

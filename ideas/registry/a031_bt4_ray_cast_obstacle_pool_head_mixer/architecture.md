...
`BT4 Primitive Mixer (ray_cast_obstacle_pool_head)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `ray_cast_obstacle_pool_head` primitive, adapted to the
...
this the cleanest available test of "is the `ray_cast_obstacle_pool_head` primitive a better

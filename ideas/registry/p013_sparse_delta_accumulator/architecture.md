# Architecture — Sparse-Delta Accumulator (p013)
...
3. **Primitive-specific accumulator state**. Implemented in
   ``compute_state`` of ``SparseDeltaAccumulator``. See
...
- Registered model name: ``sparse_delta_accumulator``.
- Source implementation: ``src/chess_nn_playground/models/primitives/sparse_delta_accumulator.py``.
- Shared helper: ``src/chess_nn_playground/models/primitives/delta_accumulator.py``.
- Idea-local wrapper: ``ideas/registry/p013_sparse_delta_accumulator/model.py``.
- Training config: ``ideas/registry/p013_sparse_delta_accumulator/config.yaml``.
- Builder entry in ``src/chess_nn_playground/models/registry.py``:
  ``MODEL_BUILDERS['sparse_delta_accumulator'] = build_sparse_delta_accumulator_from_config``.

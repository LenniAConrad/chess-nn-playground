# Ablations — Sparse-Delta Accumulator (p013)
...
``sparse_delta_accumulator.SparseDeltaAccumulator.DEFAULT_ABLATIONS``).
...
See ``sparse_delta_accumulator.SparseDeltaAccumulator.DEFAULT_ABLATIONS`` for the
complete list; the primitive-specific falsifiers documented in
``ideas/research/primitives/external_07_sparse_delta_accumulator_segment_scatter.md`` are exposed verbatim.

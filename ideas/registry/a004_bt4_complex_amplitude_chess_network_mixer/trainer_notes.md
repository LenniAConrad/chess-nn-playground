...
The CAIO mixer is more expensive per call than a 3x3 conv because each
block forms a `(B, 64, 64, amplitude_dim)` masked complex outer product
per relation (four relations total) before scattering the per-square
interference response back to the board. The shared trainer logs

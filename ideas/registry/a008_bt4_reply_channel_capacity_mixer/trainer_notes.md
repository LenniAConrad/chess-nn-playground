...
- The shared guard requires `implementation_status: implemented` (or
  ...
  `model.name == bt4_reply_channel_capacity_mixer` matching the folder
  slug.
...
- The RCC mixer adds a small amount of compute per block (two set-query
  attention pools with `K = R = 16` queries, a bilinear reply-logit table
  over `K * R * D` entries, and 24 unrolled Blahut-Arimoto iterations of
  `K * R` softmaxes plus log-marginals) over the conv baseline; the

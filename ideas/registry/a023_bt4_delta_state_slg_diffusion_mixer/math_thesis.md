...
- Mathematical motivation: The BT4-style residual tower from
  ...
  operator with the `delta_state_slg_diffusion` primitive from
  `p018_delta_state_slg_diffusion`. Source primitive math:
  `ideas/registry/p018_delta_state_slg_diffusion/math_thesis.md`.

- Assumptions:
  1. The `delta_state_slg_diffusion` primitive is well-defined as a
...
- Claimed advantage: If the `delta_state_slg_diffusion` primitive carries
  ...
  delta_state_slg_diffusion a better spatial mixer than conv or
...
- Proof sketch: This is an empirical study, not a theorem. The well-
  ...
  primitive-level math for `delta_state_slg_diffusion` itself (state-
  level diffusion over the legal-move graph with delta updates) is
...
- What is only hypothesized: That replacing the conv mixer with the
  `delta_state_slg_diffusion` mixer lifts PR AUC on at least one CRTK
  slice (most likely slices where state-level diffusion over the
  legal-move graph is load-bearing) without regressing aggregate PR AUC
  by more than the matched-baseline tolerance.

- Failure cases:
  - The `delta_state_slg_diffusion` mixer reduces to a noisy conv inside
    ...
    noise.
  - The simple_18 board tensor lacks the per-state diffusion structure
    the primitive depends on, so the mixer's diffusion signal collapses;

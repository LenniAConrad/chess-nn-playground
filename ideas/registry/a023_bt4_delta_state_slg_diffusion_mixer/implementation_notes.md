...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/delta_state_slg_diffusion.py`.
- Idea-local wrapper: `ideas/registry/a023_bt4_delta_state_slg_diffusion_mixer/model.py`.
- Registered model alias: `bt4_delta_state_slg_diffusion_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=delta_state_slg_diffusion`).
- Source primitive idea: `ideas/registry/p018_delta_state_slg_diffusion`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `delta_state_slg_diffusion`.

...
The `delta_state_slg_diffusion` mixer is more expensive per call than a
3x3 conv because it runs a state-level diffusion step over the legal-
move graph. The shared trainer logs `train_samples_per_second` to

...
- Required comparisons:
  ...
  - `i244_tempo_defender_cross_derivative_network` (A3 head-form
    control).
  - Capacity-matched `bt4_conv_mixer` (A4 control).
- Known blockers:
  - The TDCD mixer is more expensive per call than a 3x3 conv. If
...
- Target slices where this idea should beat the strongest baseline:
  `crtk_eval_bucket = equal` (the slice the source TDCD primitive was
  designed for) and `crtk_difficulty` upper tail (positions whose
  resolution depends on response asymmetry).
- Slices where this idea is expected to fail or merely match:
  ...
  slices — these are not TDCD's territory and should be measured for
  non-regression, not for lift.
- Ablation that should erase the slice-level gain: A1 (replace TDCD

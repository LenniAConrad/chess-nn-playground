...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/tempo_defender_cross_derivative_network.py`.
- Idea-local wrapper: `ideas/registry/a001_bt4_tempo_defender_cross_derivative_network_mixer/model.py`.
- Registered model alias: `bt4_tempo_defender_cross_derivative_network_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=tempo_defender_cross_derivative_network`).
- Source primitive idea: `ideas/registry/i244_tempo_defender_cross_derivative_network`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `tempo_defender_cross_derivative_network`.

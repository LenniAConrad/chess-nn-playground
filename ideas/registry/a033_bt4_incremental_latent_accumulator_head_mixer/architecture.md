...
`BT4 Primitive Mixer (incremental_latent_accumulator_head)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `incremental_latent_accumulator_head` primitive, adapted to the
...
this the cleanest available test of "is the `incremental_latent_accumulator_head` primitive a better

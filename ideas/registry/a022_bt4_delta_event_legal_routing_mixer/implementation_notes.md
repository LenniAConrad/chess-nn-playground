...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/delta_event_legal_routing.py`.
- Idea-local wrapper: `ideas/registry/a022_bt4_delta_event_legal_routing_mixer/model.py`.
- Registered model alias: `bt4_delta_event_legal_routing_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=delta_event_legal_routing`).
- Source primitive idea: `ideas/registry/p017_delta_event_legal_routing`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `delta_event_legal_routing`.

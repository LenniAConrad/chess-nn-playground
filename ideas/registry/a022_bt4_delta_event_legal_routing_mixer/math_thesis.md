...
- Mathematical motivation: The BT4-style residual tower from
  ...
  operator with the `delta_event_legal_routing` primitive from
  `p017_delta_event_legal_routing`. Source primitive math:
  `ideas/registry/p017_delta_event_legal_routing/math_thesis.md`.

- Assumptions:
  1. The `delta_event_legal_routing` primitive is well-defined as a
...
- Claimed advantage: If the `delta_event_legal_routing` primitive carries
  a spatial mixing signal that conv and attention do not, dropping it
  into the BT4 block must lift held-out PR AUC (aggregate or on a target
  slice) versus the two baselines under the same tower, optimizer, and
  data. This is a controlled architecture-level test of "is
  delta_event_legal_routing a better spatial mixer than conv or
...
- Proof sketch: This is an empirical study, not a theorem. The well-
  ...
  primitive-level math for `delta_event_legal_routing` itself (event-
  conditioned delta routing over the legal-move graph) is proven in the
  source primitive's math thesis and falsified by its own ablation grid.
...
- What is only hypothesized: That replacing the conv mixer with the
  `delta_event_legal_routing` mixer lifts PR AUC on at least one CRTK
  slice (most likely slices where event-conditioned legal-move routing
  is load-bearing) without regressing aggregate PR AUC by more than the
  matched-baseline tolerance.

- Failure cases:
  - The `delta_event_legal_routing` mixer reduces to a noisy conv inside
    the BT4 shell because the residual + SqueezeExcite path dominates
    the mixer output; the `conv` baseline matches the variant within
    noise.
  - The simple_18 board tensor lacks the per-event routing information
    the primitive depends on, so the mixer's routing signal collapses;
    an `attention` baseline matches or beats this idea.

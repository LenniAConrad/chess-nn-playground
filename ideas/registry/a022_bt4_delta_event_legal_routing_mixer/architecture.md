...
`BT4 Primitive Mixer (delta_event_legal_routing)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `delta_event_legal_routing` primitive, adapted to the
...
this the cleanest available test of "is the `delta_event_legal_routing` primitive a better

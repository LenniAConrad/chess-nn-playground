...
- Required comparisons:
  ...
  - `p017_delta_event_legal_routing` (A3 head-form control).
  - Capacity-matched `bt4_conv_mixer` (A4 control).
- Known blockers:
  - The `delta_event_legal_routing` mixer is more expensive per call
  ...
  - The simple_18 board lacks halfmove/fullmove counters; this is
    ...
    diagnostics if the mixer's internal routing depends on phase
    metadata.
...
- Target slices where this idea should beat the strongest baseline:
  slices where event-conditioned legal-move routing is load-bearing,
  most likely `crtk_tactic_motifs` involving check/capture sequences
  and `crtk_difficulty` upper tail (positions whose resolution depends
  on routing through restrictive legal-move sets).
- Slices where this idea is expected to fail or merely match:
  `mate_in_1`, quiet-positional `crtk_tactic_motifs`, and
  opening-`crtk_phase` slices — these are not where event-conditioned
  routing should dominate and should be measured for non-regression,
  not for lift.

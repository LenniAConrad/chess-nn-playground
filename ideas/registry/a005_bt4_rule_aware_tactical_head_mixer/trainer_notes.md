...
- The shared guard requires `implementation_status: implemented` (or
  ...
  `model.name == bt4_rule_aware_tactical_head_mixer` matching the folder
  slug.
...
- The mixer adds a small amount of compute per block (8 depthwise direction
  convs + a small forcing-feature MLP) over the conv baseline; the

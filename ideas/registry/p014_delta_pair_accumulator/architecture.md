# Architecture — Δ-Pair Accumulator (p014)
...
3. **Primitive-specific accumulator state**. Implemented in
   ``compute_state`` of ``DeltaPairAccumulator``. See
...
- Registered model name: ``delta_pair_accumulator``.
- Source implementation: ``src/chess_nn_playground/models/primitives/delta_pair_accumulator.py``.
...
- Training config: ``ideas/registry/p014_delta_pair_accumulator/config.yaml``.
- Builder entry in ``src/chess_nn_playground/models/registry.py``:
  ``MODEL_BUILDERS['delta_pair_accumulator'] = build_delta_pair_accumulator_from_config``.

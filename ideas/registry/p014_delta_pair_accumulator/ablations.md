# Ablations — Δ-Pair Accumulator (p014)
...
``delta_pair_accumulator.DeltaPairAccumulator.DEFAULT_ABLATIONS``).
...
See ``delta_pair_accumulator.DeltaPairAccumulator.DEFAULT_ABLATIONS`` for the
complete list; the primitive-specific falsifiers documented in
``ideas/research/primitives/external_08_delta_pair_ray_selective_bispectrum.md`` are exposed verbatim.

...
`BT4 Primitive Mixer (legal_move_graph_delta)` is a controlled architecture study, not a new
primitive. It holds the BT4-style residual tower fixed and swaps only the
per-block spatial-mixing operator.

## Implementation Binding

- Registered model name: `bt4_legal_move_graph_delta_mixer` (alias of `bt4_primitive_mixer`
...
convs. Here that mixer is replaced by the `legal_move_graph_delta` primitive, adapted to the
...
this the cleanest available test of "is the `legal_move_graph_delta` primitive a better

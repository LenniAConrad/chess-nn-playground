...
`BT4 Primitive Mixer (attack_ray_sparse_attention)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `attack_ray_sparse_attention` primitive, adapted to the
...
this the cleanest available test of "is the `attack_ray_sparse_attention` primitive a better

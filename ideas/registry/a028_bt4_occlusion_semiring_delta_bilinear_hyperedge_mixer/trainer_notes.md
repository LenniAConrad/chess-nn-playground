...
The `occlusion_semiring_delta_bilinear_hyperedge` mixer runs a
backward sweep of length `RAY_MAX_LEN = 7` along each of the 8 queen
rays (`O(64 * 7 * C)` per direction), then a per-pair bilinear
hyperedge contraction `(W_L h_left) (.) (W_R h_right)` for each of
the 4 opposite-direction pairs (`O(64 * 4 * d)` with
`d = bilinear_dim = max(8, C / 2)`), followed by a concat-and-project
fusion `Linear(4d -> C)`. The dominant per-block cost is the value
projection `V` and the per-pair `W_L`/`W_R` projections, both
`O(64 * C * d)`; the bilinear product itself is only `O(64 * 4 * d)`
after the projections. The 7-step backward recurrence is a Python
loop over depth, but the per-step body is batched across
(batch, direction, square, channel), so the wall-clock cost is
typically between the `conv` and `attention` baselines. The shared
trainer logs `train_samples_per_second` to `speed_summary.json`; if
throughput on matched hardware falls below ~40% of the `conv`
baseline, drop `model.num_blocks` or `model.channels` for the
matched comparison and re-run the conv baseline at the same shrunken
capacity.

...
`BT4 Primitive Mixer (occlusion_semiring_delta_bilinear_hyperedge)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `occlusion_semiring_delta_bilinear_hyperedge` primitive, adapted to the
...
this the cleanest available test of "is the `occlusion_semiring_delta_bilinear_hyperedge` primitive a better

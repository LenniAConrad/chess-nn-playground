...
`BT4 Primitive Mixer (occlusion_aware_ray_scan_head)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `occlusion_aware_ray_scan_head` primitive, adapted to the
...
this the cleanest available test of "is the `occlusion_aware_ray_scan_head` primitive a better

...
- Required `crtk_difficulty` breakdown: lift must concentrate on
  medium/hard buckets (where x-ray / pin / battery reasoning matters
  most) without regressing the easy bucket.
- Required `crtk_phase` breakdown: lift must hold on middlegame and
  endgame buckets (where ray-blocker interactions dominate), with no

...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/ray_semiring_chi_head.py`.
- Idea-local wrapper: `ideas/registry/a021_bt4_ray_semiring_chi_head_mixer/model.py`.
- Registered model alias: `bt4_ray_semiring_chi_head_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=ray_semiring_chi_head`).
- Source primitive idea: `ideas/registry/p016_ray_semiring_chi_head`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `ray_semiring_chi_head`.

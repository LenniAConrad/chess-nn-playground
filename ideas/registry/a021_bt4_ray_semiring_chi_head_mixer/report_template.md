...
- Required comparisons:
  ...
  - `p016_ray_semiring_chi_head` (A3 head-form control).
  - Capacity-matched `bt4_conv_mixer` (A4 control).
- Known blockers:
  - The RSCH mixer is more expensive per call than a 3x3 conv. If
  ...
  - The simple_18 board lacks halfmove/fullmove counters; this is
    ...
    if the mixer's internal saliency depends on phase metadata.
...
- Target slices where this idea should beat the strongest baseline:
  `crtk_tactic_motifs in {pin, skewer, discovered_attack, x_ray}`
  (the ray-traffic motifs the source RSCH primitive was designed for)
  and `crtk_difficulty` upper tail (positions whose resolution depends
  on long-range ray interactions).
- Slices where this idea is expected to fail or merely match:
  `mate_in_1`, knight-fork-dominated motifs, opening-`crtk_phase`
  slices — these are not RSCH's territory and should be measured for
  non-regression, not for lift.
- Ablation that should erase the slice-level gain: A1 (replace RSCH

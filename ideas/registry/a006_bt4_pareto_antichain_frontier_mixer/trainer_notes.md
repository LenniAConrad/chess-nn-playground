...
- The shared guard requires `implementation_status: implemented` (or
  ...
  `model.name == bt4_pareto_antichain_frontier_mixer` matching the folder
  slug.
...
- The PAFR mixer adds a small amount of compute per block (a set-query
  attention pool with `K = 16` queries, a small utility-head MLP, and a
  pairwise dominance product over `K^2 * C_u` entries) over the conv
  baseline; the `training.batch_size: 256` and `training.epochs: 20` budget
  defined in `config.yaml` are the comparable budget used across the

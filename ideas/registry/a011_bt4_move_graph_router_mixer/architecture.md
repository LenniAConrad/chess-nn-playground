...
`BT4 Primitive Mixer (move_graph_router)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `move_graph_router` primitive, adapted to the
...
this the cleanest available test of "is the `move_graph_router` primitive a better

...
The MGR mixer is more expensive per call than a 3x3 conv because the
implementation materialises the dense `[x_i, x_j]` concat tensor
`(B, 64, 64, 2C)` before applying the stop-grad sparse mask. The
per-edge `phi_theta` is a small two-layer GELU MLP, so the cost is
dominated by the gather + scatter at `O(B * 64^2 * C)` per block. At
the default `channels=64, num_blocks=4` the wall-clock per step is
expected to be several times the conv baseline. The shared trainer

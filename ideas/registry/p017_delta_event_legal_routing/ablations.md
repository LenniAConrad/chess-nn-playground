# Ablations — Delta-Event Legal-Move Routing (p017)
...
``delta_event_legal_routing.DeltaEventLegalRoutingHead.DEFAULT_ABLATIONS``).
...
See ``delta_event_legal_routing.DeltaEventLegalRoutingHead.DEFAULT_ABLATIONS`` for the
complete list; the primitive-specific falsifiers documented in
``ideas/research/primitives/external_11_delta_event_legal_move_routing.md`` are exposed verbatim.

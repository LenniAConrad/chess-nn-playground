# Architecture — Delta-Event Legal-Move Routing (p017)
...
3. **Primitive-specific accumulator state**. Implemented in
   ``compute_state`` of ``DeltaEventLegalRoutingHead``. See
...
- Registered model name: ``delta_event_legal_routing``.
- Source implementation: ``src/chess_nn_playground/models/primitives/delta_event_legal_routing.py``.
- Shared helper: ``src/chess_nn_playground/models/primitives/delta_accumulator.py``.
- Idea-local wrapper: ``ideas/registry/p017_delta_event_legal_routing/model.py``.
- Training config: ``ideas/registry/p017_delta_event_legal_routing/config.yaml``.
- Builder entry in ``src/chess_nn_playground/models/registry.py``:
  ``MODEL_BUILDERS['delta_event_legal_routing'] = build_delta_event_legal_routing_from_config``.

...
`BT4 Primitive Mixer (regret_saddlepoint)` is a controlled architecture study, not a new
primitive. It holds the BT4-style residual tower fixed and swaps only the
per-block spatial-mixing operator.

## Implementation Binding

- Registered model name: `bt4_regret_saddlepoint_mixer` (alias of `bt4_primitive_mixer`
...
convs. Here that mixer is replaced by the `regret_saddlepoint` primitive, adapted to the
...
this the cleanest available test of "is the `regret_saddlepoint` primitive a better

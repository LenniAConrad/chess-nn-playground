...
- The shared guard requires `implementation_status: implemented` (or
  ...
  `model.name == bt4_regret_saddlepoint_mixer` matching the folder
  slug.
...
- The RSP mixer adds a small amount of compute per block (two set-query
  attention pools with `K = R = 16` queries, a bilinear payoff over
  `K * R * D` entries, and 24 unrolled damped saddle iterations of
  `K * R` softmaxes) over the conv baseline; the

...
The `reversible_delta_kernel_memory` mixer is a linear-attention-style
kernel-memory readout per BT4 block. Its per-call cost is
`O(64 * key_dim * value_dim)` for forming `M` and `z` plus
`O(64 * key_dim * value_dim)` for the per-query readout, which is
generally cheaper than an explicit 64x64 softmax attention but more
expensive than a 3x3 conv. The shared trainer logs

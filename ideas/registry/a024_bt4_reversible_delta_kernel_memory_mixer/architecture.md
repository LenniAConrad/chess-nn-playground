...
`BT4 Primitive Mixer (reversible_delta_kernel_memory)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `reversible_delta_kernel_memory` primitive, adapted to the
...
this the cleanest available test of "is the `reversible_delta_kernel_memory` primitive a better

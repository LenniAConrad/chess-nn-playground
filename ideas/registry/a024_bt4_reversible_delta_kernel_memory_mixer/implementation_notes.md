...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/reversible_delta_kernel_memory.py`.
- Idea-local wrapper: `ideas/registry/a024_bt4_reversible_delta_kernel_memory_mixer/model.py`.
- Registered model alias: `bt4_reversible_delta_kernel_memory_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=reversible_delta_kernel_memory`).
- Source primitive idea: `ideas/registry/p019_reversible_delta_kernel_memory`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `reversible_delta_kernel_memory`.
The tower then constructs `N` `BT4MixerBlock`s, each of which builds the
named mixer through the `bt4_mixers.build_mixer` factory. The mixer is
...
The source primitive (p019) builds the kernel memory over the *active
piece set*, masking empty squares out. The BT4 spatial-mixer contract
takes an arbitrary `(B, C, 8, 8)` channel tensor with no occupancy
plane, so the mixer treats all 64 squares as active tokens. This
preserves the unordered-set semantics (`M` and `z` are still plain sums
over the token set, so signed insert/delete updates remain exact); it
just removes the per-square occupancy weighting. The math thesis flags
this as one of the documented failure modes to watch for.

...
`BT4 Primitive Mixer (pair_resonance_hessian_network)` is a controlled architecture study, not a new
...
convs. Here that mixer is replaced by the `pair_resonance_hessian_network` primitive, adapted to the
...
this the cleanest available test of "is the `pair_resonance_hessian_network` primitive a better

...
- Required comparisons:
  ...
  - `i245_pair_resonance_hessian_network` (A3 head-form control).
  - Capacity-matched `bt4_conv_mixer` (A4 control).
- Known blockers:
  - The PRHN mixer is more expensive per call than a 3x3 conv because
    it constructs a 4-vertex hypercube of perturbed inputs per
    saliency-selected pair. If throughput falls below ~40% of the conv
    baseline at scout scale, matched-budget claims become unsound; drop
    tower capacity or rewrite as additive head.
  - The simple_18 board lacks halfmove/fullmove counters; this is
    ...
    if the mixer's pair-interaction selection depends on phase metadata.
...
- Target slices where this idea should beat the strongest baseline:
  ...
  uninformative and pair-interaction structure is load-bearing) and
  `crtk_difficulty` upper tail (positions whose resolution depends on
  super-additive piece pairs like pins and forks or sub-additive
  defender/blocker relationships).
- Slices where this idea is expected to fail or merely match:
  ...
  slices — these are not PRHN's territory and should be measured for
  non-regression, not for lift.
- Ablation that should erase the slice-level gain: A1 (replace PRHN

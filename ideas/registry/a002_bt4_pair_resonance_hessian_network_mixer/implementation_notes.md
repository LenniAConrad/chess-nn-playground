...
- Mixer code: `src/chess_nn_playground/models/architecture/bt4_mixers/pair_resonance_hessian_network.py`.
- Idea-local wrapper: `ideas/registry/a002_bt4_pair_resonance_hessian_network_mixer/model.py`.
- Registered model alias: `bt4_pair_resonance_hessian_network_mixer`
  (resolved by `_bt4_alias_to_mixer` in
  `src/chess_nn_playground/models/registry.py` to
  `bt4_primitive_mixer` with `mixer=pair_resonance_hessian_network`).
- Source primitive idea: `ideas/registry/i245_pair_resonance_hessian_network`.

## Wiring

`model.py` calls `build_bt4_primitive_mixer_from_config` with
`model.mixer` defaulted to `pair_resonance_hessian_network`. The tower

...
- Mathematical motivation: The BT4-style residual tower from
  ...
  operator with the `pair_resonance_hessian_network` (PRHN) primitive
  from `i245_pair_resonance_hessian_network`. Source primitive math:
  `ideas/registry/i245_pair_resonance_hessian_network/math_thesis.md`.

- Assumptions:
  1. The PRHN primitive is well-defined as a shape-preserving operator
...
- Claimed advantage: If the PRHN primitive carries a spatial mixing
  ...
  controlled architecture-level test of "is PRHN a better spatial mixer
...
- Proof sketch: This is an empirical study, not a theorem. The well-
  ...
  primitive-level math for PRHN itself (signed pair-Hessian aggregation
  over a saliency-selected piece set, partitioning interaction mass into
  super-additive vs sub-additive components) is proven in the source
...
- What is only hypothesized: That replacing the conv mixer with the
  PRHN mixer lifts PR AUC on at least one CRTK slice (most likely the
  `crtk_eval_bucket = equal` slice and the `crtk_difficulty` upper tail
  where pair-interaction structure is load-bearing) without regressing
  aggregate PR AUC by more than the matched-baseline tolerance.

- Failure cases:
  - The PRHN mixer reduces to a noisy conv inside the BT4 shell
    because the residual + SqueezeExcite path dominates the mixer
    output; the `conv` baseline matches the PRHN variant within noise.
  - The simple_18 board tensor's piece-occupancy planes are too
    aggressively compressed by the stem conv before the mixer sees
    them, so the signed pair-Hessian collapses; an `attention` baseline
    matches or beats PRHN.
  - PRHN's per-sample cost (constructing the 4-vertex hypercube
    perturbations per saliency-selected pair) inflates wall-clock enough

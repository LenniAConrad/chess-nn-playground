# Ablations — a013 BT4 Primitive Mixer (rule_conditioned_sparse_attention)

The mixer-internal ablations
(`random_edges`, `dense_edges`, `untied_state`, `single_iteration`,
`zero_delta`, `disable_gate`, `trunk_only`) are documented in
`ideas/registry/p008_rule_conditioned_sparse_attention/ablations.md`
and are run from the primitive folder. They are reused here only as
secondary evidence about *why* the mixer helps or hurts inside the
BT4 tower.
...
| Mode | What it tests |
|---|---|
| Mixer = `rule_conditioned_sparse_attention` (this idea) | Replace the conv pair with the rule-conditioned sparse mixer inside the fixed BT4 tower. |
...
Keep `a013` over the conv baseline only if all four hold on
puzzle_binary:
...
- The lift over `bt4_attention_mixer` is at least as large as the lift
  over `bt4_conv_mixer` (rule-aware adjacency must be the source, not
  generic "attention beats conv").

Drop `a013` if any of those fail, or if the primitive-level
`random_edges` ablation matches `none` (the rule-derived adjacency was
not load-bearing).
...
- New ablation switches inside the mixer itself. Those belong in
  `p008` and would be inherited transitively.

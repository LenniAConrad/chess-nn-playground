# Idea Report Template — a013 BT4 Primitive Mixer (rule_conditioned_sparse_attention)

## Run

- Result path:
- Config: `ideas/registry/a013_bt4_rule_conditioned_sparse_attention_mixer/config.yaml`
- Seeds:
- GPU:
- Training budget:
...
| `bt4_conv_mixer` | | | | |
| `bt4_attention_mixer` | | | | |
| `bt4_rule_conditioned_sparse_attention_mixer` | | | | |

## Idea-Specific Slice Hypotheses

- Target slices where this idea should beat the strongest baseline
  (e.g. `crtk_tactic_motifs = discovered_attack`, deep attack chains
  on long open lines):
- Slices where this idea is expected to fail
  (e.g. `crtk_phase = endgame` with very low mobility):
- Ablation that should erase the slice-level gain
  (`random_edges`, `untied_state`, or `single_iteration` in the
  primitive folder):
...
- [ ] Primitive `random_edges` ablation loses the lift

If any box fails: drop `a013` from the architecture-study sweep.
...
- Highest-confidence wrong examples (FEN, `crtk_difficulty`,
  `crtk_phase`, motifs):
- Recommended next step (promote / drop / re-run with fused-scan
  kernel):

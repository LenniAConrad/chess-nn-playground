# Trainer Notes — a013 BT4 Primitive Mixer (rule_conditioned_sparse_attention)
...
- The mixer adds CPU-side adjacency construction. If wall-clock
  blows past `1.2x` the conv-mixer baseline, record it in the report
  and decide whether to defer the fused-scan kernel before re-running.

...
`BT4 Primitive Mixer (rule_conditioned_sparse_attention)` is a controlled architecture study, not a new
primitive. It holds the BT4-style residual tower fixed and swaps only the
per-block spatial-mixing operator.

## Implementation Binding

- Registered model name: `bt4_rule_conditioned_sparse_attention_mixer` (alias of `bt4_primitive_mixer`
...
convs. Here that mixer is replaced by the `rule_conditioned_sparse_attention` primitive, adapted to the
...
this the cleanest available test of "is the `rule_conditioned_sparse_attention` primitive a better

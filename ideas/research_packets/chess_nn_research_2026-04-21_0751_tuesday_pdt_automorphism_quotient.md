...
| `ideas/chatgpt_pro_deep_math_research_prompt.md` | Update | Add this packet to imported memory after implementation; add anti-duplicate rule for finite chess automorphism quotient bottlenecks if it fails. Preserve hard leakage and label constraints. |
...
For `ideas/chatgpt_pro_deep_math_research_prompt.md`, Codex should update the prompt after consuming this output. The update should preserve hard constraints while adding reusable lessons, new anti-duplicate rules, clearer output requirements, and failure-mode guidance discovered from this research pass.

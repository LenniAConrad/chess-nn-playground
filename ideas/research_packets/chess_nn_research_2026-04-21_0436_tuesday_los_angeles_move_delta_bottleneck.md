...
| Static attack-defense sheaf / Hodge / Laplacian / curvature model | Recent research packets in `ideas/research_packets/` | Explicitly disallowed as a repeated family unless the operator changes substantially; this packet avoids that family. |
...
| `ideas/chatgpt_pro_deep_math_research_prompt.md` | Update | Add this packet to researched families after implementation, with anti-duplicate guidance for move-delta bottleneck variants if it fails or succeeds. Preserve all leakage and label rules. |

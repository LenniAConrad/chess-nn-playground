...
- Registry key: `magnus_bch_coupling_series_network`.
...
- The custom linear-algebra operator described in the source packet is NOT yet
  ...
  `packet_profile=magnus_bch_coupling_series_network` so that the idea passes the registry contract
  and runs on the standard puzzle_binary benchmark. To upgrade to a bespoke module,
  add `src/chess_nn_playground/models/magnus_bch_coupling_series_network.py`, register a builder in

# Implementation Notes

- Central code: `src/chess_nn_playground/models/legal_constraint_projection_residual_network.py`.
- Idea-local wrapper: `ideas/all_ideas/registry/i134_legal_constraint_projection_residual_network/model.py`.
- Registry key: `legal_constraint_projection_residual_network`.
- Source packet: `ideas/all_ideas/research/packets/classic/chess_nn_research_2026-04-24_2136_friday_shanghai_architecture_batch_7.md`.
- Batch candidate: `Legal-Constraint Projection Residual Network`.
- This is intentionally board-only and does not consume engine, verification, source, or CRTK metadata as input.

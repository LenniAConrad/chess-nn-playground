...
`Negative-Class Disentangled Puzzle Head` is a board-only classifier
for the `puzzle_binary` task. It accepts the repository's `simple_18`
current-board tensor with shape `(B, 18, 8, 8)` and returns one puzzle
logit per position together with three explicit evidence channels and
...
2. **Pooled descriptor.** `mean(h)` and `max(h)` over the 8x8 spatial
   grid are concatenated into a `(B, 2 * channels)` vector and projected
   through a `LayerNorm -> Linear -> GELU (-> Dropout)` mixer into the
   shared evidence space `(B, evidence_dim)`.
3. **Three evidence heads.** Three independent two-layer MLPs map the
   shared evidence into scalar logits
...
4. **Single inference logit.** The puzzle logit is
   ...
   The puzzle channel must win a 3-way competition against both
   negative channels rather than against a single average negative.
5. **Auxiliary 3-way logits.** The raw `[e_random, e_near, e_puzzle]`
   stack is exposed as `aux_3way_logits` so a trainer can attach the
   packet's optional auxiliary 3-way cross-entropy on the fine source
   label (`fine 0 -> random`, `fine 1 -> near`, `fine 2 -> puzzle`).
   The current in-tree `puzzle_binary` trainer runs only the
   BCE-on-`puzzle_logit` term.
...
- Idea-local wrapper: `ideas/all_ideas/registry/i170_negative_class_disentangled_puzzle_head/model.py`.

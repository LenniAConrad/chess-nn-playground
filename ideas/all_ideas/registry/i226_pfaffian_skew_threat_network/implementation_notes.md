...
- Registry key: `pfaffian_skew_threat_network`.
- Source packet: `ideas/all_ideas/research/packets/classic/chess_nn_research_2026-05-05_1525_tuesday_local_pfaffian_skew_threat.md`.
...
- The custom linear-algebra operator described in the source packet is NOT yet
  ...
  `packet_profile=pfaffian_skew_threat_network` so that the idea passes the registry contract
  and runs on the standard puzzle_binary benchmark. To upgrade to a bespoke module,
  add `src/chess_nn_playground/models/pfaffian_skew_threat_network.py`, register a builder in

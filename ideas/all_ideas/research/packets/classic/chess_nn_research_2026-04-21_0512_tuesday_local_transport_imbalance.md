...
| `ideas/all_ideas/research/prompts/chatgpt_pro_deep_math_research_prompt.md` | Update | Add this packet to imported research memory after implementation; add anti-duplicate guidance for OT-over-piece-square transport if it fails; preserve all leakage and label rules. |
...
For `ideas/all_ideas/research/prompts/chatgpt_pro_deep_math_research_prompt.md`, Codex must update the prompt after consuming this output. The update should preserve hard constraints while adding reusable lessons from implementation and benchmarking, especially whether OT-over-piece-square mass is now an already-researched family.

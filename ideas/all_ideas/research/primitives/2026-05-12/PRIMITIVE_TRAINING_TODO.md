...
| `ideas/all_ideas/research/collections/` | Human index over primitive, classic, and registered ideas | Linked from the root idea docs |

Do not move registered `ideas/all_ideas/registry/i###_*` folders into this tree. They are the implementation registry. This primitive folder is research input until a candidate is promoted.

## Baseline Assumptions

- Baseline trunk: `i193_exchange_then_king_dual_stream`.
- Source implementation: `src/chess_nn_playground/models/exchange_then_king_dual_stream.py`.
...
1. Create or reserve the registered idea folder from `ideas/all_ideas/registry/template/`.
...
6. Add the idea-local wrapper in `ideas/all_ideas/registry/i###_<slug>/model.py`.
7. Add a config in `ideas/all_ideas/registry/i###_<slug>/config.yaml` using the same split and training protocol as i193.
8. Add focused tests under `tests/`.
9. Run static validation, smoke tests, then scout training.
10. Write run notes under `ideas/all_ideas/registry/i###_<slug>/runs/` and update `idea.yaml.latest_result_path`.

Suggested production modules:

| Primitive | Idea folder | Model module | Registry key |
|---|---|---|---|
| TSDP | `ideas/all_ideas/registry/i248_rule_aware_tactical_head/` | `src/chess_nn_playground/models/rule_aware_tactical_head.py` | `rule_aware_tactical_head` |
| PFCT | `ideas/all_ideas/registry/i246_promotion_aware_head/` | `src/chess_nn_playground/models/promotion_aware_head.py` | `promotion_aware_head` |
| TDCD | `ideas/all_ideas/registry/i244_tempo_defender_cross_derivative_network/` | `src/chess_nn_playground/models/tempo_defender_cross_derivative_network.py` | `tempo_defender_cross_derivative_network` |
| DHPE | `ideas/all_ideas/registry/i245_pair_resonance_hessian_network/` | `src/chess_nn_playground/models/pair_resonance_hessian_network.py` | `pair_resonance_hessian_network` |
| CAIO | `ideas/all_ideas/registry/i247_complex_amplitude_chess_network/` | `src/chess_nn_playground/models/complex_amplitude_chess_network.py` | `complex_amplitude_chess_network` |
...
```bash
PYTHONDONTWRITEBYTECODE=1 python scripts/validate_training_config.py --static ideas/all_ideas/registry/i###_<slug>/config.yaml
PYTHONDONTWRITEBYTECODE=1 python -m pytest tests/test_<primitive>.py
PYTHONDONTWRITEBYTECODE=1 python scripts/train_model.py --config ideas/all_ideas/registry/i###_<slug>/config.yaml
```

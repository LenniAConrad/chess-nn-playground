...
def _packet_status(path: Path, title: str, text: str) -> str:
    ...
    if re.match(r"^i25\d_", name):
        return "backlog packet"
...
def build_catalog_md(packets: list[dict[str, Any]]) -> str:
    ...
    for date in sorted(by_date.keys(), reverse=True):
        ...
        lines.append("")
    lines.append("Generated by `chess-nn-build-idea-catalog`.\n")
    return "\n".join(lines)
    ...
    if benchmark_queue:
        ...
        recommendation_lines = [
            ...
            *benchmark_queue_lines,
            ...
            "",
            "Use `reports/paper_ready_all/state.json` as the resume ledger if the trunk pipeline runner is interrupted.",

...
def _write_idea_folder(idea_id: str, row: dict[str, Any], slug: str, family: str) -> None:
    folder = Path("ideas") / f"{idea_id}_{slug}"
    ...
    idea = {
        ...
        "trainer_entrypoint": f"ideas/{idea_id}_{slug}/train.py",
        "config_path": f"ideas/{idea_id}_{slug}/config.yaml",
        "model_path": f"ideas/{idea_id}_{slug}/model.py",
        "latest_result_path": None,
        "notes": f"Research-packet promotion from `{source_path}`. This is a first-pass faithful, board-only mechanism-profile implementation intended for smoke testing and paper-grade benchmarking.",
    ...
    docs = {
        "math_thesis.md": f"# Math Thesis\n\n{name}\n\nSource packet: `{source_path}`.\n\n{candidate_rank_note}Working thesis: {thesis}\n\nThis registered implementation tests the thesis through the `{family}` mechanism profile in `ResearchPacketProbe`, using only board tensors and preserving all source/CRTK metadata for reporting.\n",
        "architecture.md": f"# Architecture\n\nThe model uses a compact board encoder, deterministic board statistics, and the `{family}` mechanism branch. It returns one puzzle logit plus diagnostics such as `packet_aux_logit`, `mechanism_energy`, `sheaf_tension`, `transport_imbalance`, `symmetry_residual`, `topology_pressure`, and `information_surprisal` when relevant.\n",
        "implementation_notes.md": f"# Implementation Notes\n\n- Central code: `src/chess_nn_playground/models/research_packet_probe.py`.\n- Registry key: `{slug}`.\n- Source packet: `{source_path}`.\n{candidate_impl_note}- This is intentionally board-only and does not consume engine, verification, source, or CRTK metadata as input.\n",
...
def main() -> None:
    catalog_path = Path("ideas/research_packets/CATALOG.jsonl")
    registry_path = Path("ideas/registry.jsonl")
    ...
    all_packet_slugs = [
        str(row.get("slug"))
        for row in registry
        if str(row.get("folder", "")).startswith("ideas/i") and Path(str(row.get("folder", ""))).exists()
        and str(row.get("source_packet_path", "")).startswith("ideas/research_packets/")
    ]

...
def sync_registry(rows: list[dict[str, Any]], path: Path = Path("ideas/registry.jsonl")) -> int:
    if not path.exists():
        return 0
...
def main() -> None:
    ...
    rows = [audit_one(folder, fix=args.fix) for folder in sorted(Path("ideas").glob("i[0-9][0-9][0-9]_*"))]

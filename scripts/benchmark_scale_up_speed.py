#!/usr/bin/env python
"""Head-to-head GPU inference benchmark: scaled-up scout architectures vs BT4.
...
Runs synthetic inference on this machine's GPU at multiple batch sizes,
measures wall-clock throughput and per-sample latency.
"""
...
def benchmark_model(name: str, model: nn.Module, sample_shape: tuple[int, ...],
                     batch_sizes: list[int], device: torch.device,
                     warmup: int = 8, iters: int = 30):
    model.eval().to(device)
    # Add dummy buffers if missing
    if device.type == "cuda":
    ...
    return {"name": name, "params": n_params, "params_fmt": fmt_M(n_params), "results": results}
...
def main() -> int:
    ...
    # ---- Build and benchmark ----
    print(f"\n=== Benchmarking on {device} at batch sizes {args.batches}, {args.iters} iters each ===\n")

    models_to_test = []
    print("Building LC0 BT4 (scaled)...")
    m_bt4 = build_lc0_bt4(**bt4_cfg)
    models_to_test.append(("LC0_BT4 (scaled, ~50M)", m_bt4, (112, 8, 8)))

    print("Building i193 dual-stream (scaled)...")
    m_i193 = build_i193_dual_stream(**i193_cfg)
    models_to_test.append(("i193_dual_stream (scaled, ~50M)", m_i193, (18, 8, 8)))
    ...
    for name, model, input_shape in models_to_test:
        ...
        try:
            r = benchmark_model(name, model, input_shape, args.batches, device, iters=args.iters)
    ...
    # ---- Print results table ----
    print("\n" + "=" * 90)
    print(f"{'model':<40s} {'params':>9s} {'batch':>6s} {'lat ms':>8s} {'samples/s':>11s}")
    print("=" * 90)
    for r in all_results:
        for b in r["results"]:
            print(f"{r['name']:<40s} {r['params_fmt']:>9s} "
                  f"{b['batch']:>6d} {b['latency_ms_per_batch']:>8.2f} "
                  f"{b['throughput_samples_per_s']:>11.0f}")
        print("-" * 90)
    return 0

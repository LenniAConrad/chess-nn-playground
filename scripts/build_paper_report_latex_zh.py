...
{\sffamily\bfseries\small\color{forest}类 0 --- 随机棋局}\\
{\scriptsize\color{muted}\texttt{6k1/p1p1Rpp1/...PP/2K5 w}}\\
{\scriptsize\color{muted}箭头: Bxd3 (随机中局采样, 非唯一谜题)}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\centering
\includegraphics[width=\linewidth]{reports/audits/puzzle_class_1.png}\\[2pt]
{\sffamily\bfseries\small\color{forest}类 1 --- 近谜题}\\
{\scriptsize\color{muted}\texttt{5rk1/6p1/2Qbp2p/.../3R2K1 w}}\\
{\scriptsize\color{muted}箭头: fxe3 (pv\_gap = 115 厘兵; 非唯一)}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\centering
\includegraphics[width=\linewidth]{reports/audits/puzzle_class_2.png}\\[2pt]
{\sffamily\bfseries\small\color{forest}类 2 --- 谜题}\\
{\scriptsize\color{muted}\texttt{5rk1/6p1/2Qbp2p/.../3R2K1 w}}\\
{\scriptsize\color{muted}箭头: dxe5 (pv\_gap = 1179 厘兵; 唯一获胜)}
\end{minipage}
\caption{每个类别一个具有代表性的样本, 使用 \texttt{crtk fen render}
渲染并配以报告调色板。三幅图均为\textbf{白方走子}。\textbf{类~1 与
类~2 的棋盘共享同一个 CRTK 姐妹父节点}
(\texttt{crtk\_parent\_-1001554678727017229}): 唯一的结构差异是, 在类~2
中黑方已经吃入 e5, 使得局面变为白方唯一战术胜利
(\texttt{dxe5}~$\to$ 吃后)。这将近谜题与真正谜题成对约束到数据所允许的
最紧密程度 --- 同一延续, 仅相差一步, 而二分标签因唯一应答性质而翻转。}
\end{figure}

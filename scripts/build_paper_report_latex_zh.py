...
{\scriptsize\color{muted}箭头: Nc4+ (随机中局采样, 非策划谜题)}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\centering
...
{\scriptsize\color{muted}箭头: exf4 (pv\_gap = 124 厘兵; 非唯一解)}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\centering
...
{\scriptsize\color{muted}箭头: Nxe5 (pv\_gap = 1036 厘兵; 唯一获胜走法)}
\end{minipage}
\caption{每个类别选取一个具有代表性的样本, 使用 \texttt{crtk fen render}
渲染。\textbf{类~1 与类~2 的棋盘共享同一个 CRTK 姐妹父节点}
(\texttt{crtk\_parent\_-1001373292417858425}): 唯一的结构差异是
\texttt{Bxe5} 是否已经走过。这使得区分近谜题与真正谜题的判别因素
缩小到数据所允许的最小程度 --- 几乎是``唯一解''性质的完美消融。}
\end{figure}

...
\clearpage
\section{提议后继: i243 (HalfKA + 双流 + LC0 头)}\label{sec:i243}
...
\begin{tcolorbox}[callout, title=请把本节当作思想实验]
下文分析的渐近极限是一个\emph{极限值}, 而非任何国际象棋引擎实际训练
所处的状态。Stockfish-eval 蒸馏使用约 $10^{7}$ 个局面; LC0 自对弈使用
约 $10^{9}$ 个局面。两者都仍在有限数据状态下, 侦察的归纳偏置在此能换来
每训练样本下真实的 Elo 提升。本节追问的是: 若数据变得无限, 会发生什么
--- 这有助于判断哪些先验值得带入更大规模的训练运行, \textbf{但并不}
代表我们的先验在实际中不再重要。
\end{tcolorbox}

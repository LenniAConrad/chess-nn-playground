...
SESSION_NAME="${RUN_ALL_SESSION:-chess-nn-all}"
VENV_DIR="${RUN_ALL_VENV:-$ROOT_DIR/.venv}"
REPORT_DIR="$ROOT_DIR/reports/paper_ready_all"
BOOTSTRAP_LOG="$REPORT_DIR/run_all_bootstrap.log"
...
if [[ -n "${RUN_ALL_LIMIT:-}" ]]; then
  runner_args+=("--limit" "$RUN_ALL_LIMIT")
fi
...
start_tmux_session() {
  ...
  if tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
    ...
    local command
    command="$(printf "%q " "$ROOT_DIR/run_all.sh" "--inside-tmux" "${runner_args[@]}")"
...
find_python() {
  if [[ -n "${RUN_ALL_PYTHON:-}" ]]; then
    python_version_ok "$RUN_ALL_PYTHON" && {
      printf '%s\n' "$RUN_ALL_PYTHON"
      return 0
    }
    return 1
...
create_or_update_venv() {
  ...
  if [[ ! -x "$VENV_DIR/bin/python" ]]; then
    ...
    local venv_args=()
    if [[ "${RUN_ALL_SYSTEM_SITE_PACKAGES:-0}" == "1" ]]; then
...
install_python_dependencies() {
  if [[ "${RUN_ALL_SKIP_INSTALL:-0}" == "1" ]]; then
    echo "Skipping Python dependency install because RUN_ALL_SKIP_INSTALL=1."
    return 0
  fi

  export PIP_DISABLE_PIP_VERSION_CHECK=1
  export PIP_ROOT_USER_ACTION=ignore
  if [[ "${RUN_ALL_USE_TSINGHUA_PIP_MIRROR:-0}" == "1" && -z "${PIP_INDEX_URL:-}" ]]; then
...
choose_gpu_ids() {
  local detected="$1"
  local requested_count="${RUN_ALL_GPU_COUNT:-}"
  local available_count
  available_count="$(count_csv_items "$detected")"

  if [[ -z "$requested_count" && "${RUN_ALL_ASK_GPU_COUNT:-0}" == "1" && "$available_count" -gt 0 && -t 0 ]]; then
  ...
  if ! [[ "$requested_count" =~ ^[0-9]+$ ]] || [[ "$requested_count" -lt 1 ]]; then
    die "RUN_ALL_GPU_COUNT must be a positive integer."
  fi
  if [[ "$available_count" -eq 0 ]]; then
    die "RUN_ALL_GPU_COUNT was set, but no visible GPUs were detected."
  fi
  if [[ "$requested_count" -gt "$available_count" ]]; then
    die "RUN_ALL_GPU_COUNT=$requested_count exceeds visible GPU count $available_count."
  fi
...
check_required_data() {
  ...
  if [[ "$missing" == "1" ]]; then
    die "Upload the canonical split files before starting the full run."
  fi
}
...
run_training() {
  ...
  if [[ -z "$gpu_ids" ]] && ! has_runner_flag "--dry-run"; then
    die "No GPU IDs detected. Set RUN_ALL_GPU_IDS=0 or fix nvidia-smi."
  fi
  ...
  local runner=(
    ...
    --seeds "${RUN_ALL_SEEDS:-42,43,44}"
    --scale-variants "${RUN_ALL_SCALE_VARIANTS:-base:1,scale_up:1.5,scale_xl:2}"
    --batch-size-caps "${RUN_ALL_BATCH_SIZE_CAPS:-base:256,scale_up:192,scale_xl:128}"
    --epochs "${RUN_ALL_EPOCHS:-30}"
    --min-epochs "${RUN_ALL_MIN_EPOCHS:-15}"
    --patience "${RUN_ALL_PATIENCE:-8}"
    --jobs "$jobs"
  )
  if [[ -n "$gpu_ids" ]]; then
    runner+=(--gpu-ids "$gpu_ids")
  fi
  if [[ -n "${RUN_ALL_TIMEOUT_MINUTES:-}" ]]; then
    runner+=(--timeout-minutes "$RUN_ALL_TIMEOUT_MINUTES")
  fi
  runner+=("${runner_args[@]}")

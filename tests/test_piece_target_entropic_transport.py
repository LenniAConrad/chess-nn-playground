...
from chess_nn_playground.models.trunk.piece_target_transport import PieceTargetEntropicTransportBottleneck
from chess_nn_playground.models.registry import build_model


IDEA_FOLDER = Path("ideas/registry/i033_piece_target_entropic_transport_bottleneck")
...
def _load_idea_model_module():
    spec = importlib.util.spec_from_file_location("i033_model", IDEA_FOLDER / "model.py")
...
def test_i033_builds_from_idea_config_and_forward_shape() -> None:
    config = _load_config()
    ...
    for key in {
        "transport_cost_forward",
        "transport_cost_reverse",
        "transport_entropy_forward",
        "transport_entropy_reverse",
        "transport_asymmetry",
        "transport_low_cost_mass",
    }:
        assert output[key].shape == (2,)
        assert torch.isfinite(output[key]).all()


def test_i033_registry_builder_matches_config_contract() -> None:
    config = _load_config()
...
def test_i033_wrapper_does_not_use_research_packet_probe() -> None:
...
def test_i033_is_bespoke_and_architecture_conformant() -> None:
    kind_row = detect_idea_implementation_kind(IDEA_FOLDER)
    assert kind_row.detected_kind == "bespoke_model"
    assert not kind_row.issues

    rows = [row for row in audit_architecture_conformance() if row.idea_id == "i033"]
    assert len(rows) == 1
    assert rows[0].implementation_kind == "bespoke_model"
    assert not rows[0].issues


def test_i033_rejects_unknown_deterministic_channel_semantics() -> None:
    with torch.no_grad():
        try:
            PieceTargetEntropicTransportBottleneck(input_channels=112, encoding_adapter="lc0_bt4_112")
